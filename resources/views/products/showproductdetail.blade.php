...
<div class="content" id="content">
    <div class="leftcontent">
        <div class="img-grid-container common-clearfix">
           {{-- {{dd($product_detail)}} --}}
           <input type="hidden" value="{{$product_detail['id']}}" id='product_id'>
            @if (isset($product_detail['productimage']['image']))
                <img src="{{ asset('images/'.$product_detail['productimage']['image']) }}" alt="" /> 
                @else
    ...
    <div class="rightcontent">
        <h3 class="pdp-title">{{$product_detail['name']}}</h3>
        <h3 class="pdp-name">{{$product_detail['description']}}</h3>
        <div class="ratebox">
          <p class="num">3.7</p>
          <img
        ...
        <div class="hr"></div>
        <div class="disline">
            <span class="pdp-name">Category:{{$product_detail['category']['category_name']}}</span>
        </div>
        <div class="disline">
          <span class="cost">Rs {{$product_detail['price']}}</span>
        </div>
        ...
        <div class="newdiv">
          <button class="cartbtn addToCartBtn" >
...
<script>
    $(document).ready(function(){
        $(".addToCartBtn").click(function(){
            let prod_id = $('#product_id').val(); //getting product id here
            $('#quantity').val(function(i, val) { return val*1+1 });
            let prod_qty = $('#quantity').val(); //getting add to cart hit value
            $('#output').html(prod_qty);
            $.ajaxSetup({
                headers: {
